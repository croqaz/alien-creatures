...
export class Panel {
  ...
  private bindButtons() {
    ...
    const spawnCreature10Btn = document.getElementById('spawn-creature-10-btn')!;
    ...
    const spawnCreature = () => {
      ...
      if (!species) return;
      const pos = randomInRect(100, 100, this.game.arena.width - 200, this.game.arena.height - 200);
    ...
    spawnFoodBtn.addEventListener('click', () => {
      for (let i = 0; i < 10; i++) {
        const pos = randomInRect(50, 50, this.game.arena.width - 100, this.game.arena.height - 100);
  ...
  updateStats() {
    ...
    const creatures = this.game.entities.filter((e: Entity) => e instanceof Creature && e.isAlive);
    const food = this.game.entities.filter((e: Entity) => e instanceof Food && e.isAlive);
